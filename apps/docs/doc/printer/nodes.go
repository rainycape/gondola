...
func (p *printer) parameters(fields *ast.FieldList) {
	...
	if len(fields.List) > 0 {
		...
		for i, par := range fields.List {
			...
			// parameter type
			p.pushState(stateInParameterType, par)
...
func (p *printer) signature(params, result *ast.FieldList) {
	...
	if n > 0 {
		...
		if n == 1 && result.List[0].Names == nil {
			...
			p.pushState(stateInParameterType, result.List[0])
...
func (p *printer) fieldList(fields *ast.FieldList, isStruct, isIncomplete bool) {
	...
	if !hasComments && srcIsOneLine {
		...
		if len(list) == 0 {
			...
			p.pushState(stateInFieldType, f.Type)
			p.expr(f.Type)
			p.popState()
	...
	if isStruct {
		...
		for i, f := range list {
			...
			if len(f.Names) > 0 {
				// named fields
				p.pushState(stateInField, f)
				p.identList(f.Names, false)
				p.popState()
				p.print(sep)
				p.pushState(stateInFieldType, f)
				p.expr(f.Type)
				p.popState()
				extraTabs = 1
			} else {
...
func (p *printer) canRef() bool {
	s, _ := p.state()
	return s == stateInValue || s == stateInParameterType || s == stateInFieldType
}

func (p *printer) prepareHref(x string, sel string) (int, string) {
	if p.HTML && p.Linker != nil && p.canRef() {
		if href := p.Linker.LinkType(x, sel); href != "" {
			return len(p.output), href
		}
	}
	return len(p.output), ""
}

func (p *printer) insertHref(pos int, href string) {
	end := len(p.output)
	for ; pos < end; pos++ {
		c := p.output[pos]
		if c != ' ' && c != '\n' && c != '\t' && c != '\v' {
	...
	tmp := make([]byte, end-pos)
	copy(tmp, p.output[pos:])
	p.output = append(p.output[:pos], beginAnchor...)
	p.output = append(p.output, href...)
	p.output = append(p.output, closeAnchor...)
	p.output = append(p.output, tmp...)
	p.output = append(p.output, endAnchor...)
}
...
func (p *printer) expr1(expr ast.Expr, prec1, depth int) {
	...
	case *ast.Ident:
		pos, href := p.prepareHref("", x.Name)
		p.print(x)
		if href != "" {
			p.insertHref(pos, href)
		}
	...
	case *ast.SelectorExpr:
		pos, href := 0, ""
		if sx, ok := x.X.(*ast.Ident); ok {
			pos, href = p.prepareHref(sx.Name, x.Sel.Name)
		}
...
func (p *printer) valueSpec(s *ast.ValueSpec, keepType bool) {
	...
	if s.Values != nil {
		...
		p.pushState(stateInValue, s.Values)
...
func (p *printer) decl(decl ast.Decl) {
	...
	case *ast.GenDecl:
		switch d.Tok {
		case token.CONST:
			p.pushState(stateInConst, d)
		case token.TYPE:
			p.pushState(stateInType, d)
		case token.VAR:
			p.pushState(stateInVar, d)
		}
		p.genDecl(d)
		if d.Tok == token.CONST || d.Tok == token.TYPE || d.Tok == token.VAR {
			p.popState()
		}
	case *ast.FuncDecl:
		p.funcDecl(d)
