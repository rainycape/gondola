...
// SendMail is a shorthand function for sending an email from a template. If you need
// more granularity you can use Context.MailTemplate and gnd.la/mail.Send directly.
// Note that fields other than Message and Data in the opts argument are not altered
// and are passed as is to gnd.la/mail.Send.
func (c *Context) SendMail(to []string, template string, data interface{}, opts *mail.Options) error {
	if template != "" {
	...
	return mail.Send(to, opts)
}

// MustSendMail works like SendMail, but panics if there's an error.
func (c *Context) MustSendMail(to []string, template string, data interface{}, opts *mail.Options) {
