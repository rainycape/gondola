...
var (
	literalWrap = []byte("<span class=\"literal\">")
	tokenWrap   = []byte("<span class=\"token\">")
	builtinWrap = []byte("<span class=\"builtin\">")
	commentWrap = []byte("<span class=\"comment\">")
	constWrap   = []byte("<span id=\"const-")
	varWrap     = []byte("<span id=\"var-")
	closeWrap   = []byte("\">")
	endWrap     = []byte("</span>")
)
...
func (p *printer) pushState(state int, data interface{}) {
	p.states = append(p.states, state)
	p.stateData = append(p.stateData, data)
}

func (p *printer) popState() (int, interface{}) {
	idx := len(p.states) - 1
	s := p.states[idx]
	data := p.stateData[idx]
	p.states = p.states[:idx]
	p.stateData = p.stateData[:idx]
	return s, data
}

func (p *printer) state() (int, interface{}) {
	idx := len(p.states) - 1
	if idx >= 0 {
		return p.states[idx], p.stateData[idx]
	}
	return 0, nil
}
...
func (p *printer) writeComment(comment *ast.Comment) {
	...
	if text[1] == '/' {
		p.writeString(pos, trimRight(text), true, typeComment)
		return
	}
	...
	for i, line := range lines {
		...
		if len(line) > 0 {
			p.writeString(pos, trimRight(line), true, typeComment)
		}
	}
}
...
func (p *printer) print(args ...interface{}) {
	for _, arg := range args {
		...
		case *ast.Ident:
			...
			if _, ok := builtinTypes[x.Name]; ok {
				dataType = typeBuiltin
			} else {
				s, _ := p.state()
				switch s {
				case stateInConst:
		...
		p.writeString(next, data, isLit, dataType)
...
func (p *printer) printNode(node interface{}) error {
	...
	} else if n, ok := node.(*ast.File); ok && n != nil {
...
type Linker interface {
	LinkType(pkg string, name string) string
	Linkify(comment string, group *ast.CommentGroup) string
}
